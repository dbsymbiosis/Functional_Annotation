...
SENSITIVITY=5.7  # mmseq2 sensitivity (1.0 faster; 4.0 fast; 7.5 sensitive) 
...
then
	...
	run_cmd "awk '\$2!=\"\"' ${KOFAM}_results.mapper.txt > ${KOFAM}_results.mapper.txt.filtered"
...
then
	...
	run_cmd "grep '>' $FASTA | sed -e 's/>//' | awk 'BEGIN{print \"#Seq_ID\tUniProt_TopHit\tGO_terms\tPFAM_Domains\tKEGG_IDs\"} {print}' | \
	 $UTILS/add_value_to_table.py --keep_comments -c 1 -d NA -a ${UNIPROT}_UniProt.sorted.outfmt6.top1.desc_annot | \
	 ...
	 $UTILS/add_value_to_table.py --keep_comments -c 1 -d NA -a ${KOFAM}_results.mapper.txt.filtered.2col -o $TABLE.tmp"
